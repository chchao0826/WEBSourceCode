...
def NoDXPlanSQL(sWorkingProcedureName):
    ...
    return "SELECT \
            ...
            ,CASE WHEN sWorkingProcedureName IN ('预定','水洗') THEN nPS2Temp3_7 ELSE nSETemp3_7 END nTemp \
            ,CASE WHEN sWorkingProcedureName IN ('预定','水洗') THEN nPSSpeed ELSE nSESpeed END AS nSpeed \
            ,CASE WHEN sWorkingProcedureName IN ('预定','水洗') THEN nPSTime  ELSE nSETime END AS nTime \
            ,sProductWidth,sProductGMWT,sColorBorder,B.uppTrackJobGUID,sWorkingProcedureNameCurrent,B.sLocation \
            ,B.sWorkingProcedureNameLast \
            ,B.sWorkingProcedureNameNext \
            ,B.sMaterialType \
